    ...
    QObject( parent )
{
    ...
    m_tray->setTitle( tr ( "Arco Settings Manager" ) );
    m_tray->setIconByName( "arco-settings-manager" );

    QMenu* menu = m_tray->contextMenu();

    QAction* msmKernel = new QAction( QIcon( ":/icons/tux-arco.png" ),
                                      tr ( "Kernels" ),
                                      menu );
    ...
    connect( msmKernel, &QAction::triggered,
             [this] ()
    {
        QProcess::startDetached( "arco-settings-manager", QStringList() << "-m" << "msm_kernel" );
        m_tray->setStatus( KStatusNotifierItem::Passive );
    } );
    connect( msmLanguagePackages, &QAction::triggered,
             [this] ()
    {
        QProcess::startDetached( "arco-settings-manager", QStringList() << "-m" << "msm_language_packages" );
        m_tray->setStatus( KStatusNotifierItem::Passive );
    } );
...
Notifier::cLanguagePackage()
{
    ...
    if ( !packages.isEmpty() )
    {
        ...
        m_tray->showMessage( tr( "Arco Settings Manager" ),
...
{
    QSettings settings( "arco", "arco-settings-manager-Notifier" );
